...
1. 로그아웃하면 저장된 workspace 설정은 유지하되 GitHub 연결 첫 화면으로 돌아갑니다.
2. 로그아웃 상태에서는 로컬 workspace 경로와 문서를 화면에 노출하지 않으며, Rust command도 workspace·document 접근을 거부합니다.
3. 재로그인하면 저장된 repository ID와 이름, 현재 계정의 접근 권한, 로컬 저장소와 `.okf/workspace.yml`을 다시 검증합니다.
4. 검증에 성공하면 기존 workspace의 Home으로 자동 복귀합니다. 이전 계정과 달라도 저장소에 접근할 수 있으면 복귀할 수 있습니다.
5. 저장소 접근 권한이 없거나 저장소 신원이 달라졌다면 공개 오류를 표시하고, 로컬 workspace가 없거나 손상됐다면 연결 복구 흐름을 표시합니다.

화면 전환은 사용자 경험을 위한 잠금이며, 실제 접근 제어는 Rust 인증 guard가 담당합니다. 로그아웃이 완료되면 이전 문서 session ID는 다시 사용할 수 없습니다.
