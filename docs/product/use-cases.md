...
1. 사용자가 GitHub App의 Device Flow로 로그인합니다. 인증 정보는 OS 보안 저장소에 보관합니다.
2. 사용자가 자신과 OkHub GitHub App이 모두 접근할 수 있는 기존 OKF 지식 저장소를 선택합니다. 새 저장소가 필요하면 GitHub에서 저장소를 생성하고, `Only select repositories` 설치라면 App 접근 대상에 추가합니다. Hub로 돌아오면 목록을 자동으로 다시 확인하며 수동 새로고침은 보조 기능으로 제공합니다.
3. 사용자가 이미 clone한 폴더를 연결하거나 새 로컬 위치를 선택합니다. 후자라면 Hub가 저장소를 clone합니다.
4. Hub가 `.okf/workspace.yml`을 읽고 워크스페이스 설정을 검증합니다.
5. 설정 파일이 없다면 생성할 파일과 대상 저장소를 미리 보여주고 초기화 승인을 받습니다.
6. 완전히 빈 저장소는 초기 커밋을 기본 브랜치에 push합니다. 기존 내용이 있는 저장소는 `okf/init-workspace` 브랜치에 commit·push하고 Draft PR을 만듭니다.
7. 사용자 기기의 저장소 경로와 UI 설정은 로컬 설정 파일에 저장합니다.
8. 연결이 완료되면 단일 워크스페이스의 Home을 엽니다. 코드 저장소와 GitHub Project는 이후 Settings에서 연결할 수 있습니다.
...
## UC-05A. 문서를 탐색하고 검색한다

1. 사용자가 Documents에 진입합니다.
2. Hub는 로컬 캐시 또는 파일 경로로 문서 트리를 먼저 표시합니다.
3. Hub는 실제 파일 metadata를 확인하고 추가·수정·삭제된 Markdown만 background에서 색인합니다.
4. 본문 색인 전에는 제목과 경로를 검색하고, 준비가 끝나면 본문 결과도 포함합니다.
5. 검색 결과에서 문서를 선택하면 해당 문서를 열고 첫 번째 일치 위치로 이동합니다.
6. 문서의 History를 열면 최근 commit을 조회하고, 선택한 과거 버전을 읽기 전용으로 표시합니다.

검색 캐시는 사용자 기기의 워크스페이스별 SQLite에 저장하는 삭제 가능한 파생 데이터입니다. 문서와 팀 공유 상태의 원본은 계속 Git이며, 읽기와 색인은 Markdown을 다시 쓰지 않습니다. frontmatter가 없거나 잘못된 문서도 가능한 범위에서 표시하고 해당 문서에만 경고를 제공합니다.
...
- Documents 상단 또는 좌측 트리에서 같은 새 문서 생성창을 엽니다. 트리에서 시작하면 선택한 폴더를, 상단에서 시작하면 마지막 사용 폴더를 기본 저장 위치로 사용합니다.
- 제목을 입력하면 읽을 수 있는 Git 파일 이름을 자동으로 제안하고 사용자가 생성 전에 수정할 수 있게 합니다.
- 사용자는 저장 위치와 기본값이 `빈 문서`인 템플릿을 확인합니다. 템플릿 목록은 검색 없이 기본 9개와 Git에 저장된 워크스페이스 템플릿을 그룹별로 보여주며 현재 선택 항목에 체크 표시를 둡니다.
- 같은 경로의 파일이 이미 있으면 자동으로 이름을 바꾸지 않고 생성 전에 충돌을 해결합니다.
- 생성 시 UUID v4 기반 문서 ID를 발급하고 템플릿의 권장 목차와 검색용 문서 타입을 복사한 뒤 리치 편집 화면을 엽니다.
