...
초기에는 `OKF 문서 저장소 1개 = 워크스페이스 1개`로 사용합니다. 저장소의 `.okf/workspace.yml`에는 워크스페이스 이름, 문서 루트, 연결된 코드 저장소 식별자, GitHub Project, 기본 브랜치와 공유 작업 규칙을 기록합니다. 사용자별 로컬 경로와 GitHub 인증 정보는 이 파일에 기록하지 않습니다.
...
| 정보 | 공유 원본 |
|---|---|
| 워크스페이스 정의 | OKF 문서 저장소의 `.okf/workspace.yml` |
...
| 현재 저장소의 로컬 경로·UI 설정 | 기기별 로컬 설정 파일 |
| push 전 문서 변경 | OKF 저장소의 Git working tree |
| 재생성 가능한 조회 캐시 | 메모리 또는 파일 시스템 캐시 |
| GitHub 인증 정보 | OS 보안 저장소 |

별도 데이터베이스 없이 Git이 워크스페이스 정의와 문서를, GitHub가 팀 공유 리뷰와 작업 상태를 보관합니다. 기기별 로컬 설정 파일에는 현재 OKF 저장소의 checkout 경로와 UI 설정만 기록합니다. push 전 문서 변경은 Git working tree에 남기고 조회 캐시는 메모리나 삭제 후 재생성할 수 있는 파일로 관리합니다.

앱은 최초 연결한 하나의 OKF 문서 저장소에서 단일 워크스페이스를 읽으며, 워크스페이스 선택 목록과 전환 기능을 제공하지 않습니다. 기본적으로 이 워크스페이스에 하나의 GitHub Project를 연결합니다. Project는 여러 코드 저장소의 Issue와 PR을 모으는 계획·진행 화면이며, 실제 작업 데이터는 Issue와 PR에 남깁니다. 초기에는 Project의 Draft Issue를 공유 원본으로 사용하지 않습니다.
