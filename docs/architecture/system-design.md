...
flowchart LR
    ...
    T --> C[공통 애플리케이션 기능]
    M --> C
    C --> K[OKF Knowledge Git 저장소]
    C --> R[백엔드·프론트엔드 코드 저장소]
    C --> G[GitHub Issue PR Review API]
    C --> L[로컬 캐시와 보안 저장소]
```
...
백엔드와 프론트엔드 저장소를 워크스페이스에 연결합니다. 하나의 Knowledge 저장소가 여러 코드 저장소와 연결될 수 있습니다.

## 공유 원본

| 정보 | 공유 원본 |
|---|---|
| 확정된 Knowledge | 기본 브랜치의 OKF Markdown |
| 검토 중인 문서 변경 | Git 브랜치와 커밋 |
| 리뷰 댓글과 승인 상태 | GitHub PR/Issue |
| 구현 코드 | 연결된 코드 저장소 |
| 구현 작업 | GitHub Issue/PR 또는 추후 합의한 작업 모델 |
...
- 워크스페이스 설정이나 Knowledge 저장소에 인증 정보 기록 금지
...
6. 승인 후 병합되면 기본 브랜치의 Knowledge가 갱신됩니다.
...
- OpenAPI 파일을 Knowledge 저장소와 백엔드 저장소 중 어디에 둘지
