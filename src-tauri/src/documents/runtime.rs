...
impl DocumentRuntime {
    ...
    pub async fn start_session(
        ...
        let result = self
            .prepare_session(session_id, workspace, cancellation.clone())
            .await;
        let (snapshot, catalog, warm_start, index_cancellation) = match result {
            Ok(prepared) => prepared,
            Err(error) => {
                self.clear_if_owned(session_id);
                return Err(error);
            }
        };
        ...
        tokio::spawn(async move {
            if warm_start {
                runtime.reconcile_filesystem(session_id).await;
            } else {
                runtime
                    .index_catalog(session_id, 0, catalog, index_cancellation, None)
                    .await;
            }
        });
        Ok(snapshot)
    ...
    pub fn stop_session(&self, session_id: Uuid) -> Result<(), AppError> {
        let mut state = self.inner.state.lock().expect("document runtime poisoned");
    ...
    pub fn snapshot(&self, session_id: Uuid) -> Result<DocumentSessionSnapshot, AppError> {
        ...
        match state.generation.as_ref() {
            Some(Generation::Active(session)) if session.session_id == session_id => {
    ...
    async fn refresh_inner(
        ...
        let (revision, cancellation) = {
            ...
            (session.index_revision, session.index_cancellation.clone())
        };
        let runtime = self.clone();
        tokio::spawn(async move {
            runtime
                .index_catalog(session_id, revision, catalog, cancellation, affected_paths)
                .await;
        });
        Ok(())
    }
    ...
    async fn reconcile_filesystem(&self, session_id: Uuid) {
        if let Err(error) = self.refresh(session_id).await {
            self.publish_if_owned(session_id, DocumentEvent::Failed { session_id, error });
        }
    }

    fn start_watcher(&self, session_id: Uuid) {
        let Some(workspace) = self.workspace_if_owned(session_id) else {
            return;
        };
        ...
        match DocumentWatcher::start(
            ...
            Ok(watcher) => {
                ...
                tokio::spawn(async move {
                    runtime
                        .watch_changes(session_id, workspace, receiver, cancellation)
                        .await;
                });
            }
            Err(_) => self.mark_watcher_degraded(session_id),
        }
    }
        ...
        loop {
            ...
            let mut paths = match message {
                ...
                WatcherMessage::BackendError => {
                    self.mark_watcher_degraded(session_id);
                    continue;
                }
            };
            ...
            if let Err(error) = self.refresh_affected(session_id, affected_paths).await {
                self.publish_if_owned(session_id, DocumentEvent::Failed { session_id, error });
            }
        }
    }

    fn mark_watcher_degraded(&self, session_id: Uuid) {
        const MESSAGE: &str =
    ...
    > {
        ...
        let cache =
            tokio::task::spawn_blocking(move || DocumentCache::open(cache_path, workspace_id))
                .await
                .map_err(join_error)?
                .map_err(cache_error)?;
        ...
        let mut state = self.inner.state.lock().expect("document runtime poisoned");
        if !matches!(state.generation.as_ref(), Some(Generation::Starting { session_id: current, .. }) if *current == session_id)
        {
            return Err(session_conflict());
        }
    ...
    async fn index_catalog(
        ...
        let to_index = {
            ...
            if session.session_id != session_id || session.index_revision != revision {
                return;
            }
            match session.cache.reconcile_metadata(&catalog.documents) {
                ...
                Err(error) => {
                    drop(state);
                    self.fail_if_owned(session_id, revision, cache_error(error));
                    return;
                }
            }
        };
        ...
        let documents = to_index
            ...
            .collect::<Vec<_>>();
        let workspace = match self.workspace_if_owned(session_id) {
        ...
        while let Some(read) = reads.recv().await {
            let body = match read.result {
                Ok(body) => body,
                Err(error) => {
                    self.fail_if_owned(session_id, revision, error);
                    continue;
                }
            };
            if !self.commit_body_if_owned(session_id, revision, &catalog, &read.summary.path, &body)
            {
                return;
            }
            indexed += 1;
        ...
        if !cancellation.is_cancelled() {
            self.set_status_if_owned(session_id, revision, IndexStatus::Ready);
        }
    }

    fn workspace_if_owned(&self, session_id: Uuid) -> Option<DocumentWorkspace> {
        let state = self.inner.state.lock().expect("document runtime poisoned");
        match state.generation.as_ref() {
            Some(Generation::Active(session)) if session.session_id == session_id => {
    ...
    fn set_status_if_owned(&self, session_id: Uuid, revision: u64, status: IndexStatus) {
        let mut state = self.inner.state.lock().expect("document runtime poisoned");
        let Some(Generation::Active(session)) = state.generation.as_mut() else {
            return;
        };
    ...
    fn fail_if_owned(&self, session_id: Uuid, revision: u64, error: AppError) {
        let state = self.inner.state.lock().expect("document runtime poisoned");
        if matches!(state.generation.as_ref(), Some(Generation::Active(session)) if session.session_id == session_id && session.index_revision == revision)
        {
            let _ = self
                .inner
                .events
                .send(DocumentEvent::Failed { session_id, error });
        }
    }

    fn publish_if_owned(&self, session_id: Uuid, event: DocumentEvent) {
        let state = self.inner.state.lock().expect("document runtime poisoned");
        if matches!(state.generation.as_ref(), Some(Generation::Active(session)) if session.session_id == session_id)
        {
            let _ = self.inner.events.send(event);
        }
    }

    fn clear_if_owned(&self, session_id: Uuid) {
        let mut state = self.inner.state.lock().expect("document runtime poisoned");
        if state
            .generation
            .as_ref()
...
mod tests {
    ...
    use super::{DocumentRuntime, DocumentSource, DocumentWorkspace};
    ...
    async fn warm_start_returns_cached_tree_before_slow_filesystem_reconciliation() {
        ...
        runtime.stop_session(first).unwrap();
    ...
    async fn missing_last_opened_document_is_cleared_from_the_next_session_snapshot() {
        ...
        runtime.stop_session(second).unwrap();
